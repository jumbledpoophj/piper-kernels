...
def _default_piper_attention_execution_plan(
    ...
    *,
    ...
    return _policy.select_execution_plan(
        ...
        key_length=query.shape[2] if key_length is None else key_length,
    )
...
def _prepare_piper_attention(  # noqa: PLR0912 - explicit preparation-plan branches
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    scale: float,
...
def _launch_piper_attention(prepared: _PreparedPiperAttention) -> torch.Tensor:
    ...
    if plan.use_sm89_d128_specialization:
        ...
        specialized_kernel[(query_length // plan.block_m, heads, batch)](
            ...
            reverse_causal_blocks=plan.optimize_causal_traversal,
